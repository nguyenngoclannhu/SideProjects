...
class QATestCases(TestCase):
    def __init__(self, query, output, chatbot=chatbot):
        super().__init__(query, output, chatbot)
        self.context = chatbot.search_documents(self.query)
        # self.context = "" if len(context) <= 0 else context
        print(f"Processing query: {query}, expected Output: {output}, Context: {self.context}")
    ...
    def create_llm_test_cases(self):
        print(f"Create   LLM Test Case for Query: {self.query}, Context: {self.context}")
...
# start = time.time()
test_cases = [tc.create_llm_test_cases() for tc in qa_test_cases]
